...
void CVideoPlayer::Prepare()
{
  ...
  if (m_pInputStream && m_pInputStream->GetIMenus())
  {
    ...
    CLog::Log(LOGNOTICE, "VideoPlayer: playing a file with a menu");
    if(std::dynamic_pointer_cast<CDVDInputStreamNavigator>(m_pInputStream))
      m_playerOptions.starttime = 0;

    if (!m_playerOptions.state.empty())
      discStateRestored = menu->SetState(m_playerOptions.state);
    else
    {
      menu->EnableSubtitleStream(m_processInfo->GetVideoSettings().m_SubtitleOn);
    }
  }
      ...
      if (st.source != STREAM_SOURCE_NONE)
      {
        if (m_pInputStream && m_pInputStream->GetIMenus() && st.source == STREAM_SOURCE_NAV)
        {
          ...
          if (menu->SetSubtitleStream(st.type_index))
          {
            ...
            int storeId = m_CurrentSubtitle.demuxerId;
            m_CurrentSubtitle.demuxerId = st.demuxerId;
            // it might take some time till the next spu packet arrives
            UpdateContentState();
            CServiceBroker::GetDataCacheCore().SignalVideoInfoChange();
            m_CurrentSubtitle.demuxerId = storeId;
            CloseStream(m_CurrentSubtitle, false);         
          }
        }
        else
...
void CVideoPlayer::SetSubtitleVisibleInternal(bool bVisible)
{
  ...
  {
    CDVDInputStream::IMenus* menu = m_pInputStream->GetIMenus();
    menu->EnableSubtitleStream(bVisible);
  }
}
...
void CVideoPlayer::UpdateContentState()
{
  ...
  if (m_pInputStream && m_pInputStream->GetIMenus())
  {
    m_content.m_videoIndex = m_SelectionStreams.TypeIndexOf(STREAM_VIDEO, STREAM_SOURCE_NAV,
    ...
    m_content.m_audioIndex = m_SelectionStreams.TypeIndexOf(STREAM_AUDIO, STREAM_SOURCE_NAV,
